...
// --------------------------------------------------
// MAFI GLOBAL ENGINE — REGIME + HYBRID + SCORE + HEATMAP
// --------------------------------------------------
...
// --------------------------------------------------
// ⭐ REGIONAL REGIME CHECK (RETURNS SCORE + OK)
// --------------------------------------------------
...
async function main() {
  ...
  const heatmapPayload = {
    ...
    regions: REGIONS.map((region) => {
      const regionFile = path.join(process.cwd(), "data", `universe_${region.name.toLowerCase()}.json`);
      let score = 0;
      let healthy = false;

      try {
        const data = JSON.parse(fs.readFileSync(regionFile, "utf8"));
        score = data.regime_score ?? 0;
        healthy = data.regime_healthy ?? false;
      } catch (err) {
        score = 0;
        healthy = false;
      }
  ...
  fs.writeFileSync(heatmapPath, JSON.stringify(heatmapPayload, null, 2), "utf8");

  console.log("--------------------------------------------------");
  console.log("GLOBAL HEATMAP GENERATED");
  console.log("Saved to:", heatmapPath);
  console.log("--------------------------------------------------");
